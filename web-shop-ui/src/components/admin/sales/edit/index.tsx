...
import { IFile } from "../create/types";
import { ProductsDialog } from "./ProductsDialog";
import { IEditSale } from "./types";

const EditSalePage = () => {
  const [state, setState] = useState<IEditSale>({
    id: 0,
    name: "",
    image: "",
    description: "",
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (state.name && state.decreasePercent && state.decreasePercent && state.expireTime) {
        ...
        if(file)
        {
            ...
            .then((resp) => {
              ...
              http.put("/api/sales", state).then((resp) => {
      ...
      <form onSubmit={handleSubmit}>
          ...
          {state.image && (
            ...
            >
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                <img
                  src={APP_ENV.IMAGE_PATH + "1200x1200_" + state.image}
        ...
        <div className="mb-6">
          ...
          <input
            ...
            id="expireTime"
            name="expireTime"
            value={state.expireTime?new Date(state.expireTime).toISOString().substring(0, 10):''}
            onChange={onChangeHandler}
            className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
            required
          />
