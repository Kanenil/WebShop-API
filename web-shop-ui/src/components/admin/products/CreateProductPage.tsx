...
import { ICategoryValue, ICreateProduct, PhotoData } from "./types";
import { Photo } from "./Photo";

export const CreateProductPage = () => {
  const [categories, setCategories] = useState<Array<ICategoryValue>>([]);

  const [state, setState] = useState<ICreateProduct>({
    name: "",
  ...
  const handleDelete = (id: string) => {
    setState((prevState) => {
      return {
        ...prevState,
        images: prevState.images.filter((image) => image.imageUrl !== id),
      };
    });
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      setState({
        ...state,
        images: [...state.images, ...updatedImages],
      });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if(state.category && state.images && state.name && state.price && parseFloat(state.price) > 0.01)
    {
